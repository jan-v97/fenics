...
#                                                setting up the mesh                                         
def get_mesh(L1,L2,theta_t,theta_r,phi,resolution):
	#calculations
...
def get_deformation(L1,L2,theta_t,theta_r,phi,resolution,delta_t,delta_r,Lt,Lr,pt,pr,tc1,tc2,tc3,tc4,tlb,tbl,edges,edges_number,W,x):
	#calculations
	rho = cos(0.5*pi-atan(1./3.)-2*phi)*sqrt(0.75*0.75+0.25*0.25)
	hr = cos(phi)-sin(phi)
	cos2phi = cos(2*phi)
	...
	edges[0].deformation = project(as_vector((((x[1]-(-0.25+L1+L2))/(-(L2+0.5)))  *   (  (rho+Lt)*sin2phi+(delta_t+hr)*cos2phi-cosphi-sinphi  -  ((rho+Lt+L2-tangens*(1-theta_t))*sin2phi+delta_t*cos2phi))  +  (rho+Lt+L2-tangens*(1-theta_t))*sin2phi+delta_t*cos2phi,((x[1]-(-0.25+L1+L2))/(-(L2+0.5)))  *   (  (rho+Lt)*cos2phi-(delta_t+hr)*sin2phi-cosphi+sinphi  -  ((rho+Lt+L2-tangens*(1-theta_t))*cos2phi-delta_t*sin2phi))  +  (rho+Lt+L2-tangens*(1-theta_t))*cos2phi-delta_t*sin2phi)),W)
	def boundary_lt(x, on_boundary):
		return on_polygon(x,edges[0], closed='c')

	edges[1].deformation = project(as_vector((tlb*((x[1])/(L1-0.75))*((x[1])/(L1-0.75))  +  ((rho+Lt)*sin2phi+(delta_t+hr)*cos2phi-cosphi-sinphi-tlb)  *  ((x[1])/(L1-0.75)) , ((x[1])/(L1-0.75))*((rho+Lt)*cos2phi-(delta_t+hr)*sin2phi-cosphi+sinphi))),W)
	def boundary_lb(x, on_boundary):
		return on_polygon(x,edges[1], closed='o')

	edges[2].deformation = project(as_vector(((x[0]/0.5)  *  (  ((rho+Lt+L2)*sin2phi+(delta_t+(1-theta_t)*hr)*cos2phi)  -  ((rho+Lt+L2-tangens*(1-theta_t))*sin2phi+delta_t*cos2phi))  +  ((rho+Lt+L2-tangens*(1-theta_t))*sin2phi+delta_t*cos2phi) , (x[0]/0.5)  *   (  ((rho+Lt+L2)*cos2phi-(delta_t+(1-theta_t)*hr)*sin2phi)  -  ((rho+Lt+L2-tangens*(1-theta_t))*cos2phi-delta_t*sin2phi))  +  (rho+Lt+L2-tangens*(1-theta_t))*cos2phi-delta_t*sin2phi)),W)
	def boundary_tl(x, on_boundary):
		return on_polygon(x,edges[2], closed='o')

	edges[3].deformation = project(as_vector((((x[0]-0.5)/0.5)  *  (  (rho+Lt+L2+tangens*theta_t)*sin2phi+(delta_t+hr)*cos2phi  -  ((rho+Lt+L2)*sin2phi+(delta_t+(1-theta_t)*hr)*cos2phi)  )  +  ((rho+Lt+L2)*sin2phi+(delta_t+(1-theta_t)*hr)*cos2phi),((x[0]-0.5)/0.5)  *  (  ((rho+Lt+L2+tangens*theta_t)*cos2phi-(delta_t+hr)*sin2phi)  -  ((rho+Lt+L2)*cos2phi-(delta_t+(1-theta_t)*hr)*sin2phi))  +  ((rho+Lt+L2)*cos2phi-(delta_t+(1-theta_t)*hr)*sin2phi))),W)
	def boundary_tr(x, on_boundary):
		return on_polygon(x,edges[3], closed='h')

	edges[4].deformation = project(as_vector((((x[1]-(0.25+L1+L2))/(-L2))  *   (  ((rho+Lt)*sin2phi+(delta_t+(1-theta_t)*hr)*cos2phi)  -  ((rho+Lt+L2)*sin2phi+(delta_t+(1-theta_t)*hr)*cos2phi))  +  (rho+Lt+L2)*sin2phi+(delta_t+(1-theta_t)*hr)*cos2phi,((x[1]-(0.25+L1+L2))/(-L2))  *   (  (rho+Lt)*cos2phi-(delta_t+(1-theta_t)*hr)*sin2phi  -  ((rho+Lt+L2)*cos2phi-(delta_t+(1-theta_t)*hr)*sin2phi))  +  ((rho+Lt+L2)*cos2phi-(delta_t+(1-theta_t)*hr)*sin2phi))),W)
	def boundary_mt(x, on_boundary):
		return on_polygon(x,edges[4], closed='c')

	edges[5].deformation = project(as_vector((((x[1]-(0.75+L1+L2))/(-(L2+0.5)))  *   (  ((rho+Lt)*sin2phi+(delta_t+hr)*cos2phi)  -  ((rho+Lt+L2+tangens*theta_t)*sin2phi+(delta_t+hr)*cos2phi))  +  (rho+Lt+L2+tangens*theta_t)*sin2phi+(delta_t+hr)*cos2phi,((x[1]-(0.75+L1+L2))/(-(L2+0.5)))  *   (  (rho+Lt)*cos2phi-(delta_t+hr)*sin2phi  -  ((rho+Lt+L2+tangens*theta_t)*cos2phi-(delta_t+hr)*sin2phi)  )  +  ((rho+Lt+L2+tangens*theta_t)*cos2phi-(delta_t+hr)*sin2phi))),W)
	def boundary_lt2(x, on_boundary):
		return on_polygon(x,edges[5], closed='o')

	edges[6].deformation = project(as_vector((tlb*((x[1]-1.)/(L1-0.75))*((x[1]-1.)/(L1-0.75))   +   ((rho+Lt)*sin2phi+(delta_t+hr)*cos2phi  -  (cosphi+sinphi)  -tlb)*((x[1]-1.)/(L1-0.75))  +  cosphi+sinphi , ((x[1]-1.)/(L1-0.75))  *  ((rho+Lt)*cos2phi-(delta_t+hr)*sin2phi  -  (cosphi-sinphi))  +  cosphi-sinphi)),W)
	def boundary_lb2(x, on_boundary):
		return on_polygon(x,edges[6], closed='c')

	edges[7].deformation = project(as_vector((tc4*((x[1]-0.25)/(L1))*((x[1]-0.25)/(L1))  +  (((rho+Lt)*sin2phi+(delta_t+(1-theta_t)*hr)*cos2phi)  -  (0.75+pr)  -  tc4)*((x[1]-0.25)/(L1))  +  0.75+pr , ((x[1]-0.25)/(L1))  *  (  (rho+Lt)*cos2phi-(delta_t+(1-theta_t)*hr)*sin2phi  -  (0.25+pt)  )  +  0.25+pt)),W)
	def boundary_c4(x, on_boundary):
		return on_polygon(x,edges[7], closed='o')

	edges[8].deformation = project(as_vector((tc3*((x[1]-0.25)/0.75)*((x[1]-0.25)/0.75)  +  (cosphi+sinphi  -  (0.75+pr)  -  tc3)*((x[1]-0.25)/0.75)  +  0.75+pr,((x[1]-0.25)/0.75)  *  (  cosphi-sinphi  -  (0.25+pt)  )  +  0.25+pt)),W)
	def boundary_c3(x, on_boundary):
		return on_polygon(x,edges[8], closed='o')

	edges[9].deformation = project(as_vector((((x[0]-0.75)/(L1-0.75))  *  (  Lr  -  (0.75+pr)  )  +  0.75+pr,tc2*((x[0]-0.75)/(L1-0.75))*((x[0]-0.75)/(L1-0.75))  +  (0.5*hr*theta_r+delta_r - (0.25+pt) - tc2)*((x[0]-0.75)/(L1-0.75))  +  0.25+pt)),W)
	...
	edges[11].deformation = project(as_vector(((x[0]/L1)*(Lr),tbl*(x[0]/L1)*(x[0]/L1)  +  (-0.5*hr*theta_r+delta_r  -  tbl)*(x[0]/L1))),W)
	def boundary_bl(x, on_boundary):
		return on_polygon(x,edges[11], closed='c')

	edges[12].deformation = project(as_vector((x[0]+(Lr-L1),-0.5*hr*theta_r+delta_r)),W)
	def boundary_br(x, on_boundary):
		return on_polygon(x,edges[12], closed='o')

	edges[13].deformation = project(as_vector((((x[0]-L1)/(L2+0.5))  *  (L2+theta_r*(cosphi+sinphi))  +  Lr,0.5*hr*theta_r+delta_r)),W)
	def boundary_mr(x, on_boundary):
		return on_polygon(x,edges[13], closed='c')

	edges[14].deformation = project(as_vector((((x[1]+0.25)/(0.5))  *  theta_r*(cosphi+sinphi) + Lr+L2,((x[1]+0.25)/(0.5))  *  (0.5*hr*theta_r+delta_r  - (-0.5*hr*theta_r+delta_r))  - 0.5*hr*theta_r+delta_r)),W)
	def boundary_rb(x, on_boundary):
		return on_polygon(x,edges[14], closed='h')

	edges[15].deformation = project(as_vector((((x[1]-0.25)/(0.5))  * (1-theta_r)*(cosphi+sinphi)  +  Lr+L2+theta_r*(cosphi+sinphi),((x[1]-0.25)/(0.5))  *  ((1-theta_r)*hr)  + 0.5*hr*theta_r+delta_r)),W)
	def boundary_rt(x, on_boundary):
		return on_polygon(x,edges[15], closed='o')

	edges[16].deformation = project(as_vector((((x[0]-(L1+1.))/(L2))  *  L2  +  Lr+cosphi+sinphi,(1-0.5*theta_r)*hr+delta_r)),W)
	def boundary_br2(x, on_boundary):
		return on_polygon(x,edges[16], closed='c')

	edges[17].deformation = project(as_vector((((x[0]-1.)/L1)*(Lr)+cosphi+sinphi,tbl*((x[0]-1.)/L1)*((x[0]-1.)/L1)  +  ((1-0.5*theta_r)*hr+delta_r  -  (cosphi-sinphi)  -  tbl)*((x[0]-1.)/L1)  +  cosphi-sinphi)),W)
...
#                                                 the programm                                        
def do_shape_opt(L1,L2,theta_t,theta_r,phi,resolution,a1,a2,a3,a4):
	#calculations
	rho = cos(0.5*pi-atan(1./3.)-2*phi)*sqrt(0.75*0.75+0.25*0.25)
	hr = cos(phi)-sin(phi)
	cos2phi = cos(2*phi)
	...
	mesh, edges, edges_number, chi_a, chi_b, chi_c, chi_d, chi_test, verts = get_mesh(L1,L2,theta_t,theta_r,phi,resolution)
	...
	class PeriodicBoundary (SubDomain):
		...
		def map (self, x, y): y[0] = x[0]-(cosphi+sinphi); y[1] = x[1]-(cosphi-sinphi)
	...
	psi, dpsi = get_deformation(L1,L2,theta_t,theta_r,phi,resolution,Delta_t,Delta_r,Lt,Lr,pt,pr,tc1,tc2,tc3,tc4,tlb,tbl,edges,edges_number,W,x)

	GA = Constant (((1,0.1),(0,1)))
	GB = Constant (((1,-0.1),(0,1)))
	GC = Constant (((1,0),(0.1,1)))
	GD = Constant (((1,0),(-0.1,1)))
	...
	Ehat = ReducedFunctional(assemble(E), [Control(Delta_t),Control(Delta_r),Control(Lt),Control(Lr),Control(pt),Control(pr),Control(tc1),Control(tc2),Control(tc3),Control(tc4),Control(tlb),Control(tbl)])

	#          Taylor Test:                 
	h = [Constant(6.7),Constant(0.1),Constant(0.1),Constant(0.1),Constant(0.1),Constant(0.1),Constant(0.1),Constant(0.1),Constant(0.1),Constant(0.1),Constant(0.1),Constant(0.1)]
	conv_rateL = taylor_test(Ehat, [Delta_t,Delta_r,Lt,Lr,pt,pr,tc1,tc2,tc3,tc4,tlb,tbl], h)
	# Computed convergence rates: [1.942568351785441, 1.989259352956199, 2.028272338072]
	...
	psi, dpsi = get_deformation(L1,L2,theta_t,theta_r,phi,resolution,float(rDelta_t),float(rDelta_r),float(rLt),float(rLr),float(rpt),float(rpr),float(rtc1),float(rtc2),float(rtc3),float(rtc4),float(rtlb),float(rtbl),edges,edges_number,W,x)
...
E_end,Delta_t,Delta_r,Lt,Lr,pt,pr,tc1,tc2,tc3,tc4,tlb,tbl, chi_test, dpsi, u,verts = do_shape_opt(L1,L2,theta_t,theta_r,phi,resolution,a1,a2,a3,a4)
