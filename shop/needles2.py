...
class PeriodicBoundary (SubDomain):
	...
	def inside (self, x, on_boundary): 
		return (on_polygon(x,edges[0]) and on_boundary) or (on_polygon(x,edges[1]) and on_boundary)  or ((-tol<x[0]<L1) and (near(x[1],x[0]*(-0.25/7.)))) or (on_polygon(x,edges[12]) and on_boundary) 
