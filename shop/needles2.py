...
def get_deformation(L1,L2,theta_t,theta_r,phi,alpha,resolution,delta_t,delta_r,Lt,Lr,pt,pr,tc1,tc2,tc3,tc4,tlb,tbl,edges,edges_number,V,x):
	...
	B = 4.
	B = sin(alpha-atan((y1_y-(0.25+pt))/(y1_x-(0.75+pr))))*A
	#B = sin(alpha-atan2(y1_y-(0.25+pt),y1_x-(0.75+pr)))*A
	point_a_x = (0.75+pr + (Lt+L2) * x1_x - (theta_t*C-(sin(alpha-atan((y1_y-(0.25+pt))/(y1_x-(0.75+pr))))*sqrt((y1_x-(0.75+pr))**2+(y1_y-(0.25+pt))**2)+delta_t)) * x2_x  + (theta_t-1)*y1_x)
	point_a_y = (0.25+pt + (Lt+L2) * x1_y - (theta_t*C-(sin(alpha-atan((y1_y-(0.25+pt))/(y1_x-(0.75+pr))))*sqrt((y1_x-(0.75+pr))**2+(y1_y-(0.25+pt))**2)+delta_t)) * x2_y  + (theta_t-1)*y1_y)
	point_b_x = (0.75+pr + Lt * x1_x + (sin(alpha-atan((y1_y-(0.25+pt))/(y1_x-(0.75+pr))))*sqrt((y1_x-(0.75+pr))**2+(y1_y-(0.25+pt))**2)+delta_t) * x2_x  -  y1_x)
	point_b_y = (0.25+pt + Lt * x1_y + (sin(alpha-atan((y1_y-(0.25+pt))/(y1_x-(0.75+pr))))*sqrt((y1_x-(0.75+pr))**2+(y1_y-(0.25+pt))**2)+delta_t) * x2_y  -  y1_y)
	...
	print("break1")
	edges[0].deformation = project(as_vector(((x[0]+pt,x[1]+pr))),V)
	def boundary_lt(x, on_boundary):
		return on_polygon(x,edges[0], closed='c')
	
	print("break2")
	edges[1].deformation = project(as_vector(((x[0]+pt,x[1]+atan(1/pr)))),V)
...
def do_shape_opt(L1,L2,theta_t,theta_r,phi,alpha,resolution,a1,a2,a3,a4,sDelta_t,sDelta_r,sLt,sLr,spt,spr,stc1,stc2,stc3,stc4,stlb,stbl,G_tl,G_tr,G_rt,G_rb,taylor_testing,ftol,gtol,method,maxiter,mesh, edges, edges_number, chi_a, chi_b, chi_c, chi_d):
	...
	if (taylor_testing == 0):
		...
		bounds = [[-1000,-1000,0.75,0.75,-0.75,-0.25,-1000,-1000,-1000,-1000,-1000,-1000],[1000,1000,1000,1000,0.25,0.75,1000,1000,1000,1000,1000,1000]]
		Ehat = ReducedFunctional(assemble(E), controls,derivative_cb_post = derivative_cb,eval_cb_post = eval_cb_post)
